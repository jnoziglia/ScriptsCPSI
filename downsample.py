...
for folder in folders:
    ...
    for fname in mat_files:
        ...
        val = mat['ECG'].data[1]
        (downsampled_signal, downsampled_signal_t) = wfdb.processing.resample_sig(x=val, fs=500, fs_target=200)
        ...
        for start in range(segments):
            ...
            scipy.io.savemat(fname.split('.')[0] + '_' + str(start) + '.mat', segment_dict)
        if downsampled_signal.size % SAMPLES != 0:
            start = len(downsampled_signal) - SAMPLES
            segment = downsampled_signal[start:]
            segment_dict = {'ECG': segment}
            scipy.io.savemat(fname.split('.')[0] + '_' + str(segments) + '_DA.mat', segment_dict)